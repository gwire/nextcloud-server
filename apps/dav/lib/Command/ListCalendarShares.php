...
class ListCalendarShares extends Command {
	...
	protected function configure() {
		$this
			->setName('dav:list-calendar-shares')
			->setDescription('List calendar shares')
			->addArgument('uid',
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if (count($calendarShareTableData) > 0) {
			$table = new Table($output);
			$table->setHeaders(['uri', 'type', 'principal', 'displayname', 'status', 'writable'])
				->setRows($calendarShareTableData);
			$table->render();
		} else {
			$output->writeln("<info>Calendar <$calname> has no shares</info>");
		}
		return 0;
	}
